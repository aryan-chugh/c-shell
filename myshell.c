...
bool process_commands(char **list, int *codes, dir_tree *tree, log_deque *ldeq, char *s);
...
// cmd has no cmd part... 
void my_runner(char **list, int *codes, log_deque *ldeq, dir_tree *tree, int indx, char *cmd) {
    cmd = minimise_spaces(cmd);
    switch(indx) {
        case 1:  // hop  
        ...
        case 3: // log
            ...
            if(rval != -1 && rval != 0) {
                bool should_add = process_commands(list, codes, tree, ldeq, cmd_to_execute);
                if(should_add) {
...
void run_command(char **list, int *codes, log_deque *ldeq, dir_tree* tree, char *cmd, bool bg, int out_fd, int inp_fd) {
    cmd = minimise_spaces(cmd);
    int ppid = getpid();
    int status;
    ...
    if(pid == -1) {
        ...
        if((ind = find_in_predefined_list(list, codes, command)) == -1) {
            ...
            my_runner(list, codes, ldeq, tree, ind, cmd_wo);
        }
...
bool process_commands(char **list, int *codes, dir_tree *tree, log_deque *ldeq, char *s) {    
    ...
    while(t[c] != NULL) {
        ...
        // printf("%s\n", command);
        run_command(list, codes, ldeq, tree, t[c], m[c], output_fd, input_fd);
...
char *print_exceeds() {
    ...
    for (int i = 0; i < f_count; i++) {
        ...
        snprintf(to_print + current_length, buffer_size - current_length, "%s : %d%s", f_process[i], f_times[i], (i == f_count - 1) ? " : " : " ; ");
